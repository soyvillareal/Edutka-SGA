...
if($one == 'get-foitems'){
    ...
    if(isset($id) && is_numeric($id)){
        ...
        if(empty($emptys)){
            ...
            $course_id = $dba->query('SELECT course_id FROM notes WHERE id = '.$qualification['note_id'])->fetchArray();
            $users = $dba->query('SELECT user_id FROM teacher WHERE course_id = '.$course_id.' AND period_id = '.$qualification['period_id'])->fetchAll(false);
            $users[] = $qualification['user_id'];
            ...
            if(empty($errors)){
                if($dba->query('UPDATE qualification SET note = ?, status = ? WHERE id = '.$id, $note, $status)->returnStatus()){
                    ...
                    if($status != 'pending' && $status != $qualification['status']){
                        foreach ($users as $user) {
                            $notify_exists = $dba->query('SELECT COUNT(*) FROM notifications WHERE from_id = '.$TEMP['#user']['id'].' AND to_id = '.$user.' AND course_id = '.$course_id.' AND type = "quate_accepted"')->fetchArray();
                            $type = $user == $qualification['user_id'] ? "st_$status" : ($notify_exists > 0 ? "te_rejected" : "te_accepted");
                            if($status != 'rejected' || $user == $qualification['user_id'] || $notify_exists > 0){
                                Specific::SendNotification(array(
                                    'from_id' => $TEMP['#user']['id'],
                                    'to_id' => $user,
                                    'course_id' => $course_id,
