...
if ($TEMP['#loggedin'] === true) {
	...
	if ($type == 'new') {
		...
		$deliver['status'] = 304;
		$notifications = $dba->query('SELECT * FROM notification WHERE to_id = '.$TEMP['#user']['id'].' ORDER BY id DESC LIMIT 10')->fetchAll();
		if(!empty($notifications)){
			$site_title = $TEMP['#settings']['title'];
			foreach ($notifications as $value) {
				$user_data = Specific::Data($value['from_id']);
				$notifycon = $TEMP['#notifycon'][$value['type']];
				$course = $dba->query('SELECT name FROM course WHERE id = '.$value['course_id'])->fetchArray();
			...
			$dba->query('UPDATE notification SET seen = '.time().' WHERE to_id = '.$TEMP['#user']['id']);
			$deliver = array(
				'status' => 200,
				'html' => $html
			);
